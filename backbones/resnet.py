...
from transforms.torchvision_present import ImageClassification

from ._api import register_model, Weights, WeightsEnum
from ._meta import _IMAGENET_CATEGORIES
from ._utils import _ovewrite_named_param, handle_legacy_interface, _log_api_usage_once
import copy
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        act_dict = {}
        if type(x) == dict: # keys should be "final" and "all_acts"
            act_dict.update(x['all_acts'])
            x = x['final']
        identity = x
...
class ResNet(nn.Module):
    def __init__(
        ...
        self.layer1 = self._make_layer(block, 64, layers[0], block_id=1)
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2, dilate=replace_stride_with_dilation[0], block_id=2)
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2, dilate=replace_stride_with_dilation[1], block_id=3)
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2, dilate=replace_stride_with_dilation[2], block_id=4)
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        layer2_outs = self.layer2(layer1_outs['final']) # block 2
        layer3_outs = self.layer3(layer2_outs['final']) # block 3
        layer4_outs = self.layer4(layer3_outs['final']) # block 4


        output_dict.update(layer1_outs['all_acts'])
        output_dict.update(layer2_outs['all_acts'])
        output_dict.update(layer3_outs['all_acts'])
        output_dict.update(layer4_outs['all_acts'])

        # imagenetr50_pre_logits_pre_pooling_8192 same as imagenetr50_block4_unit3_layer2_8192?
